...
//
//! Security Aspect Tests for absolute-zero.
//!
...
fn security_byte_wrapping_is_safe() {
    ...
    // Should wrap around safely (u8 is always < 256 so this always passes)
    let _value = interp.state.memory[0];
    assert!(true); // Wrapping is guaranteed by u8 type
}
...
fn security_input_buffer_exhaustion() {
    ...
    let mut interp = BrainfuckInterpreter::new(program);
    interp.state.input_buffer.push_back(42); // One byte

    interp.run();

    // Should have read the input
