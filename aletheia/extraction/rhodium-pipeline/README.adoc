...
* *Enforces* RSR build requirements (zero dependencies, zero unsafe,
etc.)
...
Rhodium Pipeline generates CI/CD configurations that enforce RSR
compliance automatically. It works with rhodibot to verify compliance
...
. *RSR-First* - Enforces RSR requirements by default
...
Checks: - Required stages present - RSR checks configured - Security
scans enabled - No unsafe operations
