...
//
//! Property-Based Tests for absolute-zero.
//!
...
fn property_balanced_operations_are_cno() {
    ...
    // Note: "++>--<" modifies cell 0 before moving, so final state may differ
    let test_cases = vec![
        "+-",
        "+-+-+-",
        "++++----",
...
fn property_idempotent_operations() {
    // Test that equivalent programs have the same CNO property
    let test_pairs = vec![
        ("+-", "+-"),     // Same program, definitely equivalent
        (">< test", "<>"), // Comments ignored, both balanced
